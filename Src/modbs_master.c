...
void mbInit(lpMbParam_t mbParam)
{
		...
		float end_packet_delay = (4 * 10 * 1000000L)/baudrate;					// +/- время на 4 символа в микросекундах
...
void mbParsePacket( lpMbParam_t mbParam )
{
		...
		if( (mbParam->function == 0x03) && (mbParam->rxCount  == ((mbParam->num_regs)*2 + 5)))				// если читаем 0x03, то отает должен быть адрес(1) + функция(1) + количество(1) + CRC(2) + (num_regs*2)		байт
		{	
